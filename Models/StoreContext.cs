...
namespace doan.Models
{
    public class StoreContext
    {
       ...
       *  SQL Dat Hang 
       *--------------------------------*/
        public int insert_DDH(int maKH, int tongtien, DateTime ngay, int nvc)
        {
            using (SqlConnection conn = GetConnection())
            {
                conn.Open();
                var str = "insert into DonDatHang(MAKH,MAVOUCHER,TONGDONHANG,SOTIENGIAM,THANHTIEN,MANV,NGAYDATHANG,MANVC) " +
                    "values(@makh,1,@thanhtien,0, @thanhtien,1, @ngaydat, @nvc)";
                SqlCommand cmd = new SqlCommand(str, conn);
